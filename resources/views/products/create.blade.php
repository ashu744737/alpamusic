...
@section('title') {{ trans('form.products_form.add_product') }} @endsection
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-4 col-xs-12">
                            <h4 class="card-title">{{ trans('form.products_form.add_product') }}</h4>
                        </div>
                    </div>
                    @include('session-message')
                    <div class="row mt-3">
                        <div class="col-12">
                            <form action="{{ route('product.store') }}" method="POST" id="create_product" class="form needs-validation" enctype="multipart/form-data">
                                @csrf
                               
                                <div class="form-row">
                                    <div class="form-group col-md-12">
                                        <label for="name" class="ul-form__label">{{ trans('form.products_form.product_name') }} <span class="text-danger">*</span> </label>
                                        <input  type="text" class="form-control" id="name" name="name" placeholder="{{ trans('form.products_form.product_name') }}" value="{{ old('name') }}"  />
                                        @error('name')
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-12">
                                        ...
                                        <input  type="number" class="form-control" id="price" name="price" placeholder="{{trans('form.products_form.price')}}" value="{{ old('price') }}" step="0.01" required/>
                                        @error('price')
                                        <span class="invalid-feedback d-block" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                    ...
                                    <div class="form-group col-md-12">
                                        ...
                                        <select name="category_id" id="category_id" class="form-control @error('category_id') is-invalid @enderror" required>
                                            @foreach($categories as $id => $category)
                                                <option value="{{ $category['id'] }}" {{ old('category_id') == $category['id'] ? 'selected' : '' }}>{{ App::isLocale('hr')?$category['hr_name']:$category['name'] }}</option>
                                            @endforeach
                                        </select>
                                        @error('category_id')
                                ...
                                <div class="form-row dc-div" style="display: none">
                                    <div class="form-group col-md-12">
                                        ...
                                        <input  type="number" class="form-control" id="delivery_cost" name="delivery_cost" placeholder="{{ trans('form.products_form.del_cost') }}" value="{{ old('delivery_cost') }}"  step="0.01" required/>
                                        @error('delivery_cost')
                                        <span class="invalid-feedback d-block" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
