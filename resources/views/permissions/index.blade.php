...
@section('title') {{ trans('form.permissions_form.permissions') }} @endsection
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-4 col-xs-12">
                            <h4 class="card-title">{{ trans('form.permissions_form.permissions') }}</h4>
                        </div>
                        @if(check_perm('permission_create'))
                        <div class="col-12 col-sm-8 col-xs-12 text-left text-sm-right mb-2">
                            <a href="{{ route('permission.create') }}" class="btn btn-primary w-md waves-effect waves-light add-new-btn">{{ trans('form.permissions_form.add_permission') }}</a>
                        </div>
                        @endif
                    </div>

                    @include('session-message')
                    @if(check_perm('permission_show'))
                    <div class="row">
                        <div class="col-12">
                            ...
                            <table id="datatable_clients" class="table table-bordered dt-responsive">
                                ...
                                <tr role="row">
                                    ...
                                    <th>{{ trans('form.permissions_form.permission_name') }} </th>
    ...
    <script>
        $(document).ready(function() {
            ...
            function ajaxRemoveRecord(record_id) {
                ...
                $.ajax({
                    url: "permissions/remove/" + record_id,
                    method: 'delete',
                ...
                if(rows_selected.length > 0){
                    Swal.fire({
                      ...
                      if (result.value) {
                        ...
                        $.ajax({
                            url: "{{ route('permission.multidelete') }}",
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                "ids": ids,
