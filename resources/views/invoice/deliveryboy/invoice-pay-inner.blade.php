            ...
            <div class="row">
                <div class="col-12">                 
                    ...
                    <div class="row">
                        <div class="col-6">
                            <address>
                                <strong>{{ trans('general.paid_to') }}</strong> {{$invoice->deliveryboy->user->name ?? '-' }}<br>
                                {{$invoice->deliveryboy->user->userAddresses[0]->address2 ?? '-' }}<br>
                                {{ $invoice->deliveryboy->user->userAddresses[0]->city.',' ?? '-' }}{{ $invoice->deliveryboy->user->userAddresses[0]->state.',' ?? '-' }}<br>
                                {{ App::isLocale('hr')? $invoice->deliveryboy->user->userAddresses[0]->country->hr_name:$invoice->deliveryboy->user->userAddresses[0]->country->en_name }}{{ '-'.$invoice->deliveryboy->user->userAddresses[0]->zipcode ?? '-' }}
                            </address>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="">
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                    @php
                                        $docCost = \App\DeliveryboyInvestigations::where('deliveryboy_id', $invoice->deliveryboy_id)->where('investigation_id', $invoice->investigation_id)->first();
                                        @endphp
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    <tbody>
                                        ...
                                        @if($documents->documenttype->include_vat && $documents->uploaded_by == \App\Helpers\AppHelper::getUserIdFromDeliveryboyId($docCost->deliveryboy_id) && $documents->investigation_id == $docCost->investigation_id)
                                        <tr>
                                            ...
                                            @php $doccost+=$documents->documenttype->price; @endphp
                                        </tr>
                                        @endif
                                        @endforeach
                                        @endif

                                        @if(isset($notIncludeVat) && $notIncludeVat)
                                        <tr>
                                            <td colspan="3">
                                        ...
                                        @foreach($invoice->investigation->documents as $key => $documents)
                                            @if($documents->documenttype->include_vat == 0 && $documents->uploaded_by == \App\Helpers\AppHelper::getUserIdFromDeliveryboyId($docCost->deliveryboy_id) && $documents->investigation_id == $docCost->investigation_id && !empty($documents->documenttype->price))
                                            <tr>
                                                <td>{{$documents->doc_name}} </td>
                                                @if(config('app.locale') == 'hr')
                                                <td>{{$documents->documenttype->hr_name}}</td>
                                        ...
                                        <tr>
                                         ...
                                         <tr>
                                            ...
                                            <td class="no-line text-right">
                                                ...
                                                <h4 class="m-0">{{ trans('form.investigationinvoice.grand_total') }} {{ trans('general.money_symbol') }}{{(($invoice->amount - $invoice->discount_amount)+$docCost->doc_cost)}}</h4>
                                                ...
                                                @else
                                                    ...
                                                    <h4 class="m-0">{{ trans('form.investigationinvoice.grand_total') }} {{ trans('general.money_symbol') }}{{(($invoice->amount - $invoice->parital_amount)+$docCost->doc_cost)}}</h4>
                                                    @else
                                                        @if($invoice->status != 'paid' && (isSM() || isAdmin()))
                                                            <h4 class="m-0">{{-- trans('form.investigationinvoice.grand_total') --}} {{-- trans('general.money_symbol') --}} 
                                                                {{--$invoice->amount ? $invoice->amount+$docCost->doc_cost:'0' --}}
                                                                {{ trans('form.investigationinvoice.grand_total') }} {{ trans('general.money_symbol') }} 
                                                                {{$invoice->amount ? $invoice->amount:'0'}}

                                                            <input type="hidden" id="amount" class="amount" name="amount" value="{{$invoice->amount ? $invoice->amount+$docCost->doc_cost:'0' }}" step="0.01" data-id="{{ $invoice->id }}" required style="font-weight: initial;height: 25px;width: 130px;" onchange="calAmt();"></h4>
                                                        @else
                                                            <h4 class="m-0">{{ trans('form.investigationinvoice.grand_total') }} {{ trans('general.money_symbol') }}{{$invoice->amount ? $invoice->amount:'0'}}</h4>
                                                        @endif
                                                    @endif
                                                @endif
                                            </td>
                                ...
                                <div class="table-rep-plugin">
                                    <div class="table-responsive mb-0" data-pattern="priority-columns">
                                        <table id="inve_documents" class="table table-bordered">
                                            ...
                                            @php
                                            $viewable = (isAdmin() || isSM() || isAccountant() || (isDeliveryboy()));
                                            @endphp
                                            @if($viewable)
                                            <tr>
                                            @php $isimage=0;
                                            ...
                                            $imgurl='/deliveryboy-invoice-pay-docs/'.$document->file_name;
                                            @endphp
                                            <td>{{$indx}}</td>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                ...
                                action="{{ route('deliveryboy-invoice-pay') }}"
                                ...
                                @csrf
                                    ...
                                    <input type="hidden" value="{{ $invoice->deliveryboy->user->id }}" name="user_id" />
                                    @if($invoice->status != 'paid' && (isSM() || isAdmin()))
