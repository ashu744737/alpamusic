...
@section('title') {{ trans('form.registration.investigation.investigation_details') }} @endsection
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-12 col-sm-5 col-xs-12">
                        <h4 class="card-title pb-4">{{ trans('form.registration.investigation.investigation_details') }}
                            @if(!empty($invn->status))
                                @php $statusbadge='';
                                    if($invn->status=='Open')
                                    $statusbadge='warning';
                                ...
                                @if(isClient())
                                    ...
                                    @endphp
                                    <span class="badge dt-badge badge-{{ $statusbadge }}"> {{ $status }} </span>
                                @else
                                <span class="badge dt-badge badge-{{ $statusbadge }}"> {{ trans('form.investigation_status.'.str_replace(" ","",$invn->status)) }} </span>
                                @endif
                            @endif
                            @if ((isClient() && $invn->status == 'Pending Approval') || isAdmin() || isSM())
                            <a href="{{ route('investigation.edit', [Crypt::encrypt($invn->id)]) }}"  title="{{trans('general.edit') }}">
                            <span class="badge dt-badge badge-primary"> <i class="fas fa-edit"></i> </span>  </a>
                            @endif
                            @if($invn->is_urgent)
                                <span class="badge dt-badge badge-danger"> {{trans('general.Urgent')}}</span>
                            @endif
                        </h4>
                    </div>
                    ...
                    <!-- Actions for Admin -->
                    @if((isAdmin() || isSM()) && $invn->status != 'Closed')
                        <div class="actiondropdown col-12 col-sm-7 col-xs-12 text-sm-right">
                            <div class="dropdown dropdown-topbar d-inline-block">
                                ...
                                <div class="dropdown-menu action-dd" aria-labelledby="dropdownMenuLink">
                                    @if(check_perm('investigation_approve') && $invn->status != 'Closed' && ($invn->status == 'Pending Approval' || $invn->status == 'Approved' || $invn->status == 'Declined'))
                                        @if($invn->status != 'Approved' && $assigned->isEmpty() && $assignedDel->isEmpty())
                                            <a onclick="approveconfirm({{$invn->id}})" class="dropdown-item" href="javascript:void(0);">{{ trans('general.approve') }}</a>
                                        @endif
                                        @if($invn->status != 'Declined' && $assigned->isEmpty() && $assignedDel->isEmpty())
                                            <a onclick="opendeclineModal('decline_model','decline_data',{{$invn->id}})" class="dropdown-item action-decline" data-toggle="modal"  href="javascript:void(0);">{{ trans('general.decline') }}</a>
                                        @endif
                                        @if($invn->status != 'Approved' || $invn->status != 'Declined')
                                            <div class="dropdown-divider"></div>
                                        @endif
                                    @endif
                                    @if(check_perm('investigation_assign') && $invn->status != 'Closed')
                                        @if(( $invn->status != 'Pending Approval' && $invn->status != 'Declined' ) && ( $assigned->isEmpty() || ( $assigned->isNotEmpty() && !$assigned->contains('status', 'Report Accepted') && !$assigned->contains('status', 'Final Report Submitted') ) ))
                                            <a href="{{ route('investigation.show-search-investigators', [Crypt::encrypt($invn->id)]) }}" class="dropdown-item">{{ trans('form.investigation.assign_investigator') }}</a>
                                        @endif
                                        @if($typeofInq->product->is_delivery == '1' && $assigned->isNotEmpty() && $assigned->contains('status', 'Report Accepted') && ( $assignedDel->isEmpty() || ($assignedDel->isNotEmpty() && !$assignedDel->contains('status', 'Done And Delivered')) ))
                                            <a href="{{ route('investigation.show-search-deliveryboys', [Crypt::encrypt($invn->id)]) }}" class="dropdown-item">{{ trans('form.investigation.assign_deliveryboy') }}</a>
                                            <div class="dropdown-divider"></div>
                                            <a href="{{ route('investigation.show-search-investigators', [Crypt::encrypt($invn->id)]) }}" class="dropdown-item">{{ trans('form.investigation.assign_investigator') }}</a>
                                        @endif
                                    @endif

                                    @if($invn->status != 'Closed' && $invn->status != 'Send To Client' && $assigned->isNotEmpty() && $assigned->contains('status', 'Report Accepted'))
                                        @if( ($typeofInq->product->is_delivery == '1' && $assignedDel->isNotEmpty() && $assignedDel->contains('status', 'Done And Delivered')) || ($typeofInq->product->is_delivery == '0' || empty($typeofInq->product->is_delivery)))
                                            <a onclick="smReportEvent()" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.SubmitFinalReport') }}</a>
                                        @endif
                                    @endif

                                    @if($invn->status == 'Writing Report' && $invn->status != 'Closed' && $invn->status != 'Finalizing Report' && $invn->status != 'Printing' && $invn->status != 'Waiting For Final Approval' && $invn->status != 'Send To Client' && $assigned->isNotEmpty() && $assigned->contains('status', 'Final Report Submitted'))
                                        @if( ($typeofInq->product->is_delivery == '1' && $assignedDel->isNotEmpty() && $assignedDel->contains('status', 'Done And Delivered')) || ($typeofInq->product->is_delivery == '0' || empty($typeofInq->product->is_delivery)))
                                            @php $transStatus = trans('form.timeline_status.Finalizing Report') @endphp
                                            <a onclick="changeAssignmentStatus('Finalizing Report', 'admin', '{{$transStatus}}')" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.FinalizingReport') }}</a>
                                        @endif
                                    @endif

                                    @if($invn->status == 'Finalizing Report' && $assigned->isNotEmpty() && $assigned->contains('status', 'Final Report Submitted'))
                                        @if( ($typeofInq->product->is_delivery == '1' && $assignedDel->isNotEmpty() && $assignedDel->contains('status', 'Done And Delivered')) || ($typeofInq->product->is_delivery == '0' || empty($typeofInq->product->is_delivery)))
                                            @php $transStatus = trans('form.timeline_status.Printing') @endphp
                                            <a onclick="changeAssignmentStatus('Printing', 'admin', '{{$transStatus}}')" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.Printing') }}</a>
                                        @endif
                                    @endif

                                    @if($invn->status == 'Printing' && $assigned->isNotEmpty() && $assigned->contains('status', 'Final Report Submitted'))
                                        @if( ($typeofInq->product->is_delivery == '1' && $assignedDel->isNotEmpty() && $assignedDel->contains('status', 'Done And Delivered')) || ($typeofInq->product->is_delivery == '0' || empty($typeofInq->product->is_delivery)))
                                            @php $transStatus = trans('form.timeline_status.Waiting For Final Approval') @endphp
                                            <a onclick="changeAssignmentStatus('Waiting For Final Approval', 'admin', '{{$transStatus}}')" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.WaitingForFinalApproval') }}</a>
                                        @endif
                                    @endif

                                    @if($invn->status == 'Waiting For Final Approval' && $assigned->isNotEmpty() && $assigned->contains('status', 'Final Report Submitted'))
                                        @if( ($typeofInq->product->is_delivery == '1' && $assignedDel->isNotEmpty() && $assignedDel->contains('status', 'Done And Delivered')) || ($typeofInq->product->is_delivery == '0' || empty($typeofInq->product->is_delivery)))
                                            @php $transStatus = trans('form.timeline_status.Send To Client') @endphp
                                            <a onclick="changeAssignmentStatus('Send To Client', 'admin', '{{$transStatus}}')" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.SendToClient') }}</a>
                                        @endif
                                    @endif

                                    @if($invn->status == 'Send To Client' && $assigned->isNotEmpty() && $assigned->contains('status', 'Final Report Submitted'))
                                        @if( ($typeofInq->product->is_delivery == '1' && $assignedDel->isNotEmpty() && $assignedDel->contains('status', 'Done And Delivered')) || ($typeofInq->product->is_delivery == '0' || empty($typeofInq->product->is_delivery)))
                                            @php $transStatus = trans('form.timeline_status.Closed') @endphp
                                            <a onclick="changeAssignmentStatus('Closed', 'admin', '{{$transStatus}}')" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.Closed') }}</a>
                                        @endif
                                    @endif

                                    @if($assigned->isNotEmpty() && !$assigned->contains('status', 'Report Accepted'))
                                        <div class="dropdown-divider"></div>
                                    @endif
                    ...
                    <div class="loading col-12 col-sm-7 col-xs-12 text-sm-right d-none"><h4><span class="badge badge-primary">{{ trans('general.updating') }}</span></h4></div>
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title mt-0">{{trans('form.timeline.decline_reason')}}</h5>
                                ...
                                <div class="modal-body">
                                    <form class="reportdeclineform" id="reportdeclineform">
                                        ...
                                        <div class="col-md-12 col-lg-12 col-xl-12">
                                            ...
                                            <textarea rows="7" id="decline_reason" name="decline_reason" type="textarea" placeholder="{{trans('form.timeline.decline_reason')}}" class="form-control" ></textarea>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-12 col-lg-12 col-xl-12 ">
                                        ...
                                        <button id="reportDecline" type="button" onclick="" class="btn btn-danger waves-effect" data-status="" data-performer="" data-entityId="" data-textstatus="" data-assignid="">{{{ trans('general.decline') }}}</button>
                    ...
                    @if(isInvestigator())
                        @if($assigned->isNotEmpty())
                            @if($assigned->first()->status == 'Report Declined' || $assigned->first()->status == 'Returned To Investigator')
                                @if(!is_null($assigned->first()->decline_by))
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                        ...
                                        <div class="col-md-12 col-lg-12 col-xl-12">
                                            <div class="form-group">
                                            <textarea rows="7" id="decline_reason" name="decline_reason" type="textarea" placeholder="{{trans('form.timeline.decline_reason')}}" class="form-control" readonly>{{$assigned->first()->decline_reason ?? trans('general.no_reason')}}</textarea>
                                            </div>
                                        </div>
                        ...
                        <div class="actiondropdown col-12 col-sm-8 col-xs-12 text-sm-right">
                            <div class="dropdown dropdown-topbar d-inline-block">
                                ...
                                <div class="dropdown-menu action-dd" aria-labelledby="dropdownMenuLink">
                                    ...
                                    @if($invn->status != 'Closed' && $assigned->isNotEmpty() && $assigned->contains('status', 'Report Accepted'))
                                        @if( ($typeofInq->product->is_delivery == '1' && $assignedDel->isNotEmpty() && $assignedDel->contains('status', 'Done And Delivered')) || ($typeofInq->product->is_delivery == '0' || empty($typeofInq->product->is_delivery)))
                                            <a onclick="smReportEvent()" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.SubmitFinalReport') }}</a>
                                        @endif
                                    @endif
                    ...
                    @if(isInvestigator() && $assigned->isNotEmpty())
                        @if(($assigned->first()->status != 'Final Report Submitted' && $assigned->first()->status != 'Completed With Findings' && $assigned->first()->status != 'Report Accepted'))
                        <div class="actiondropdown col-12 col-sm-7 col-xs-12 text-sm-right">
                            @if($assigned->isNotEmpty())
                                @if($assigned->first()->status == 'Report Declined' || $assigned->first()->status == 'Returned To Investigator')
                                    @if(!is_null($assigned->first()->decline_by))
                                        <button type="button" class="btn btn-warning w-md waves-effect waves-light" id="showDeclineReason">
                                            {{trans('form.timeline.decline_reason')}}
                                        </button>
                                    @endif
                                @endif
                            @endif
                            <div class="dropdown dropdown-topbar d-inline-block">
                                ...
                                <div class="dropdown-menu action-dd" aria-labelledby="dropdownMenuLink">
                                    ...
                                    @if( (!$assigned->contains('status', 'Report Accepted') && !$assigned->contains('status', 'Final Report Submitted') ) && $invAssigned && $invAssigned->status != 'Investigation Declined' && $invAssigned->status != 'Completed Without Findings' && $invAssigned->status != 'Completed With Findings' && $invn->status != 'Closed' && $invAssigned->status != 'Cancel')

                                        @if($invAssigned->status == 'Assigned')
                                        <a onclick="showAcceptDeclineModal()" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.InvestigationAcceptDecline') }}</a>
                                        @endif

                                        @if($invAssigned->status != 'Assigned')
                                        <a onclick="finishAssignmentEvent()" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigator_investigation_status.complete') }}</a>
                                        @endif
                    ...
                    @endphp
                    @if(isDeliveryboy() && $assignedDel->isNotEmpty())
                        @if($delAssigned->status != 'Return To Center' && $delAssigned->status != 'Done And Not Delivered' && $invn->status != 'Closed' && $delAssigned->status != 'Cancel')
                        <div class="actiondropdown col-12 col-sm-7 col-xs-12 text-sm-right">
                            <div class="dropdown dropdown-topbar d-inline-block">
                                <a class="btn btn-primary" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    {{ trans('general.action') }} <i class="mdi mdi-chevron-down"></i>
                                ...
                                <div class="dropdown-menu action-dd" aria-labelledby="dropdownMenuLink">
                                    @if(!is_null($delAssigned) && $delAssigned->status != 'Done And Not Delivered' && $invn->status != 'Closed' && $delAssigned->status != 'Cancel')

                                        @if($delAssigned && $delAssigned->status == 'Assigned')
                                            <a onclick="showDelAcceptDeclineModal()" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigation_status.InvestigationAcceptDecline') }}</a>
                                        @endif

                                        @if($delAssigned && $delAssigned->status == 'Accepted')
                                            <a onclick="finishAssignmentEventDel()" class="dropdown-item" href="javascript:void(0)" >{{ trans('form.investigator_investigation_status.complete') }}</a>
                                        @endif
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <ul class="nav nav-tabs nav-tabs-custom nav-justified mb-2" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" data-toggle="tab" href="#basic_details" role="tab">
                                    ...
                                    <span class="d-none d-sm-block">{{ trans('form.registration.investigation.basic_details') }}</span>
                                </a>
                            </li>
                            @endif
                            @php $indeltabactive='';
                            if(isInvestigator() || isDeliveryboy())
                            {$indeltabactive='active';}@endphp
                            <li class="nav-item">
                                <a class="nav-link  {{$indeltabactive}}" data-toggle="tab" href="#subjects" role="tab">
                                    <span class="d-block d-sm-none"><i class="far fa-file-alt"></i></span>
                                    <span class="d-none d-sm-block">{{ trans('form.registration.investigation.subjects') }}</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab" href="#documents" role="tab">
                                    <span class="d-block d-sm-none"><i class="far fa-file-alt"></i></span>
                                    <span class="d-none d-sm-block">{{ trans('form.registration.investigation.documents') }}</span>
                                </a>
                            </li>
                            @if(!isClient())
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab" href="#timeline" role="tab">
                                    ...
                                    <span class="d-none d-sm-block">{{ trans('form.registration.investigation.time_line') }}</span>
                                </a>
                            </li>
                            @endif
                            {{-- <li class="nav-item">
                                <a class="nav-link" data-toggle="tab" href="#history" role="tab">
                                    ...
                                    <span class="d-none d-sm-block">{{ trans('form.registration.investigation.history') }}</span>
                                </a>
                            </li> --}}
                        </ul>
                        ...
                        <div class="tab-content">
                            @if(!isInvestigator() && !isDeliveryboy())
                            <!-- BASIC DETAILS -->
                            @include('investigation.partials.basic')
                            @endif

                            <!-- Subjects -->
                            @include('investigation.partials.subject')

                            <!-- DOCUMENTS -->
                            @include('investigation.partials.document')

                            <!-- TIMELINE -->
                            @include('investigation.partials.timeline')

                            <!-- HISTORY -->
                           {{--  @include('investigation.partials.history') --}}
...
<div class="modal fade bd-example-modal-lg" id="finishAssignmentModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ trans('form.investigator_investigation_status.complete_inv') }} </h5>
            ...
            <div class="modal-body">
                <form class="add_mobile" id="completion_form" novalidate="novalidate" method="post" action="{{ route('investigation.complete-investigation') }}">

                    @csrf
                    <input type="hidden" name="invnid" id="invnid" value="{{ $invn->id }}"/>
                    <input type="hidden" name="invrid" id="invrid" value="{{ Auth::id() }}"/>

                    <div class="form-row check-formraw mb-2">

{{--                            <div class="form-group col-md-12">--}}
{{--                                <label for="consent">{{ trans('form.investigation.select_finding_type') }}</label>--}}
{{--                            </div>--}}

{{--                            <div class="form-group col-md-6">--}}
{{--                                <div class="custom-control custom-radio">--}}
{{--                                    <input type="radio" class="custom-control-input" id="complete_findings" name="complete_type" value="1" checked>--}}
{{--                                    <label class="custom-control-label" for="complete_findings">{{ trans('form.investigator_investigation_status.complete_findings') }}</label>--}}
{{--                                </div>--}}
{{--                            </div>--}}
                        ...
                        <div class="form-group col-md-6">
                            <div class="custom-control custom-checkbox">
                                ...
                                <label class="custom-control-label" for="complete_without_findings">{{ trans('form.investigator_investigation_status.complete_without_findings') }}</label>
                            </div>
                        </div>
                    ...
                    <div class="form-row with-finding-fields hs-fields py-2">
                        @foreach($invn->subjects as $subject)
                            <div class="form-group col-md-12">
                                <label>{{ trans('form.investigation.write_summary_of') . ' ' . trans('form.registration.investigation.subject') . ' ' . $subject->first_name }} : </label>
                                <textarea class="form-control note" name="summary[{{$subject->id}}]" placeholder="{{ trans('form.investigation.write_note_sub') }}" ></textarea>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="form-row with-finding-fields">
                        <div class="form-group col-md-12 mt-2">
                            <label>{{ trans('form.investigation.final_summary') }} : </label>
                            <textarea class="form-control note" name="final_summary" placeholder="{{ trans('form.investigation.write_final_summary') }}" ></textarea>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary completeButton">{{trans('form.investigator_investigation_status.complete')}}</button>
...
<div class="modal fade bd-example-modal-lg" id="finishAssignmentModalDel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="add_mobile" id="completion_form_del" novalidate="novalidate" method="post" action="{{ route('investigation.complete-investigation-del') }}">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary completeButtonDel">{{trans('form.investigator_investigation_status.complete')}}</button>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans('general.cancel')}}</button>
            </div>
        </div>
    </div>
...
<div class="modal fade bd-example-modal-lg" id="acceptDeclineModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ trans('form.investigator_investigation_status.accept_inv') }} </h5>
                    ...
                    <form>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                @endphp
                                    {{ trans('form.registration.investigation.inv_cost_paid') }} : {{ trans('general.money_symbol')}}<span id="invn_invr_cost"> @if(!is_null($investigator)){{ $investigator->inv_cost }}@endif</span>
                                    
                                    
                                    {{--<span id="invn_invr_cost">{{ \App\Helpers\AppHelper::calculateInvestigationCostForInvestigator($invn->id, \App\Helpers\AppHelper::getInvestigatorIdFromUserId(Auth::id())) }}</span>--}}
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <label>{{ trans('form.investigator_investigation_status.note_by_sm') }} : </label>
                                @php
                                    $note = "";
                                    if(isInvestigator()){
                                ...
                                @endphp
                                <textarea class="form-control note" name="note" readonly>{{ $note }}</textarea>
                            </div>
                        </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-success acceptButton">{{trans('general.accept')}}</button>
                <button type="button" class="btn btn-danger rejectButton">{{trans('general.reject')}}</button>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans('general.cancel')}}</button>
            </div>
        </div>
    </div>
...
<div class="modal fade bd-example-modal-lg" id="acceptDeclineDelModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label for="invn_cost" class="mb-0">
                                    ...
                                    {{-- {{ trans('form.registration.investigation.inv_cost_paid') }} : {{ trans('general.money_symbol')}}<span id="invn_invr_cost">{{ \App\Helpers\AppHelper::calculateInvestigationCostForDeliveryboy($invn->id, \App\Helpers\AppHelper::getDeliveryboyIdFromUserId(Auth::id())) }}</span> --}}
                                    @php
                                    ...
                                    @endphp
                                    {{ trans('form.registration.investigation.inv_cost_paid') }} : {{ trans('general.money_symbol')}}<span id="invn_invr_cost">@if(!is_null($delivery)){{ $delivery->inv_cost }}@endif</span>
                                </label>
                            </div>
                        </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-success acceptDelButton">{{trans('general.accept')}}</button>
                <button type="button" class="btn btn-danger rejectDelButton">{{trans('general.reject')}}</button>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans('general.cancel')}}</button>
            </div>
        </div>
    </div>
...
<div class="modal fade bd-example-modal-lg" id="rejectReasonModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ trans('form.investigator_investigation_status.RejectInvestigation') }} </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form class="reject-form">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            <label>{{ trans('form.reject_reason') }} : </label>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-danger submitRejectButton">{{trans('general.reject')}}</button>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans('general.cancel')}}</button>
            </div>
        </div>
    </div>
...
<div class="modal fade bd-example-modal-lg" id="smReportModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ trans('form.investigation_status.SubmitFinalReport') }} </h5>
            ...
            <div class="modal-body">
                <form class="add_mobile" id="completion_form_sm" novalidate="novalidate" method="post" action="{{ route('investigation.submit-sm-report') }}">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary completeButtonSM">{{trans('general.submit')}}</button>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans('general.cancel')}}</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    $(function () {
            ...
            init: function () {
                ...
                this.on("success", function (file, response) {
                    ...
                    if(response.data.fileId){
                        ...
                        $.ajax({
                            url: '{{ route("investigation.update-documenttype") }}',
                            type: "POST",
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "doctype": doctypeval,
                        ...
                        if(isadminsm==1) {
                            ...
                            trHTML+='<div class="form-inline form-group justify-content-between">' +
                                '<label for="share_with_client_' + response.data.fileId + '" class="form-check-label">{{trans('form.client_reg')}}</label>' +
                                ...
                                '<label class="ml-2" for="share_with_client_' + response.data.fileId + '" data-on-label="{{ trans('general.yes') }}" data-off-label="{{ trans('general.no') }}"> </label>' +
                                '</div>';
                            trHTML+='<div class="form-inline form-group justify-content-between">' +
                                '<label for="share_with_investigator_' + response.data.fileId + '" class="form-check-label">{{ trans('form.investigator_reg') }}</label>' +
                                ...
                                '<label class="ml-2" for="share_with_investigator_' + response.data.fileId + '" data-on-label="{{ trans('general.yes') }}" data-off-label="{{ trans('general.no') }}"> </label>' +
                                '</div>';
                            trHTML+='<div class="form-inline form-group justify-content-between">' +
                                '<label for="share_with_delivery_boy_' + response.data.fileId + '" class="form-check-label">{{ trans('form.delivery_boy_reg') }}</label>' +
                                ...
                                '<label class="ml-2" for="share_with_delivery_boy_' + response.data.fileId + '" data-on-label="{{ trans('general.yes') }}" data-off-label="{{ trans('general.no') }}"> </label>' +
                                '</div>';
                            trHTML+= '</td>'
                        }
                        ...
                        {{--Swal.fire("{{trans('general.uploaded_text')}}", "{{trans('form.registration.investigation.document_uploaded')}}", "success");--}}
                    }
        ...
        $(".completeButton").on('click', function(e){
            ...
            let status = $("input[name='complete_type']").is(":checked") ? "{{trans('form.timeline_status.Completed Without Findings')}}" : "{{trans('form.timeline_status.Completed With Findings')}}";

            Swal.fire({
                title: "{{trans('general.are_you_sure')}}",
                text: "{{trans('form.investigation.change_status_msg')}}" + status + '?',
                type: "warning",
                    ...
                    $.ajax({
                        ...
                        success: function(response)
                        {
                            ...
                            if (response.status == 'success') {
                                Swal.fire({
                                    text: "{{trans('general.success_text')}}", 
        ...
        $(".acceptButton").on('click', function(){
            let status = "{{trans('form.timeline_status.Investigation Started')}}";
        ...
        $('.submitRejectButton').on('click', function(){
            $.ajax({
                type: "POST",
                url: "{{route('investigation.reject')}}",
                data: {
                    "_token": "{{ csrf_token() }}",
                ...
                success: function(data){
                    if(data.result){
                        @if(isInvestigator())
                            let status = "{{trans('form.timeline_status.Investigation Declined')}}";
                            changeAssignmentStatus('Investigation Declined', 'investigator', status);
                        @else
                            let status = "{{trans('form.timeline_status.Rejected')}}";
                            changeAssignmentStatus('Rejected', 'deliveryboy', status);
                        @endif
                    }
                }, error: function(data){
                    Swal.fire("{{trans('general.error_text')}}", "{{trans('general.something_wrong')}}", "error");
                }
            });
        });

        $(".acceptDelButton").on('click', function(){
            let status = "{{trans('form.timeline_status.Accepted')}}";
        ...
        $(".completeButtonDel").on('click', function(e){
            ...
            let txtStatus = "{{trans('form.timeline_status.Return To Center')}}";

            Swal.fire({
                title: "{{trans('general.are_you_sure')}}",
                text: "{{trans('form.investigation.change_status_msg')}}" + txtStatus + '?',
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#34c38f",
                cancelButtonColor: "#f46a6a",
        ...
        $(".completeButtonSM").on('click', function(e){
            ...
            let txtStatus = "{{trans('form.timeline_status.Final Report Submitted')}}";
    ...
    function deleteDocument(id, obj) {
        Swal.fire({
            title: "{{trans('general.are_you_sure')}}",
            text: "{{trans('form.registration.investigation.document_confirm_delete')}}",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#34c38f",
            cancelButtonColor: "#f46a6a",
                        ...
                        success: function (response) {
                            if (response.status == 'success') {
                                Swal.fire({
                                    ...
                                    text: (result.message) ? result.message : "{{trans('form.registration.investigation.document_deleted')}}", 
                                    type: "success",
                                    confirmButtonText: "{{ trans('general.ok') }}",
                                });
                                ...
                                if ($('#inve_documents >tbody >tr').length < 1) {
                                    var trHTML = '<tr> <td colspan="3" class="text-center"> {{ trans('form.registration.investigation.document_notfound') }} </td> </tr>';
                                    $('.tr-nodoc').show();
                                }
                            } else {
                                Swal.fire("{{trans('general.error_text')}}", (result.message) ? result.message : "{{trans('general.something_wrong')}}", "error");
        ...
        $.ajax(
            {
                ...
                success: function (response) {
                    if (response.status == 'success') {
                        Swal.fire({
                            title: "{{trans('general.success_text')}}",
                            text: (response.message) ? response.message : "{{trans('form.registration.investigation.confirm_statuschanged')}}", 
                            type: "success",
                            confirmButtonText: "{{trans('general.ok')}}",
                        });
                    } else {
    ...
    function notPaidAlert() {
        Swal.fire({
            title: "{{trans('general.payment_not_done')}}",
            type: "warning",
    ...
    function approveconfirm(id) {
        ...
        Swal.fire({
            title: "{{trans('general.are_you_sure')}}",
            text: "{{trans('form.registration.investigation.approve_confirm')}}",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#34c38f",
            cancelButtonColor: "#f46a6a",
            confirmButtonText: "{{trans('general.yes_approve')}}",
            cancelButtonText: "{{trans('general.cancel')}}"
        }).then(function (result) {
            if (result.value) {
                    ...
                    success: function (response) {
                        if (response.status == 1) {
                            Swal.fire({
                                ...
                                text: (result.msg) ? result.msg : "{{trans('form.registration.investigation.approve_confirmed')}}", 
                                type: "success",
                                confirmButtonText: "{{ trans('general.ok') }}",
                            })
                                .then((result) => {
    ...
    function changeAssignmentStatus(status, performer, translatedStatus, entityId = null) {
        ...
        Swal.fire({
            title: "{{trans('general.are_you_sure')}}",
            text: "{{trans('form.investigation.change_status_msg')}}" + translatedStatus + '?',
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#34c38f",
            cancelButtonColor: "#f46a6a",
                                    ...
                                    .then((result) => {
                                        if (result.value) {
                                            if(status == "Send To Client"){
                                                location.href = "{{ route('investigation.showinvoice', \Illuminate\Support\Facades\Crypt::encrypt($invn->id)) }}";
    ...
    function changeispaymentdocument(e,subid){
        ...
        acflag=1;
            $.ajax({
                        url: '{{ route("investigation.updateispaymentdoc") }}' ,
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            docId: subid,
    ...
    function changeToUrgent(id) {
        Swal.fire({
            title: "{{trans('general.are_you_sure')}}",
            text: "{{trans('form.registration.investigation.urgent_confirm')}}",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#34c38f",
            cancelButtonColor: "#f46a6a",
    ...
    $('#reportDecline').on('click', function(){
        ...
        Swal.fire({
            title: "{{trans('general.are_you_sure')}}",
            text: "{{trans('form.investigation.change_status_msg')}}" + textstatus + '?',
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#34c38f",
            cancelButtonColor: "#f46a6a",
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "{{route('investigation.action-on-report-decline')}}",
                    data: {
                        assignId: assignid,
    ...
    $('.invst_amount').editable({
        type: 'text',
        url: "{{ route('investigator-invoice-amount') }}",
    ...
    $('.delboy_amount').editable({
        type: 'text',
        url: "{{ route('delboy-invoice-amount') }}",
        name: 'investigator_cost',
        title: 'Enter username',
        mode: 'inline',
        inputclass: 'form-control-sm',
