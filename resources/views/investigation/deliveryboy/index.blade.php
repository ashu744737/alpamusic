                    ...
                    <div class="col-12">
                        ...
                        <div class="list-group collapse" id="investigation_status">
                            ...
                            <ul class="nav nav-pills nav-justified investigation_status mt-lg-2 py-lg-3 mt-0 py-0"
                                role="tablist">
                                <li class="nav-item">
                                    <a class="nav-link active" data-toggle="tab" href="#inv_all" role="tab" onclick="changeDTTab('datatable-investigation-deliveryboy', '')">
                                        <span class="tab_title">{{ trans('form.deliveryboy_investigation_status.All') }}</span>
                                        <span class="badge badge-light count-all">{{ $invCounts['All'] ?? '0' }}</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" data-toggle="tab" href="#inv_inprogress" role="tab" onclick="changeDTTab('datatable-investigation-assign', 'In Progress')">
                                        <span class="tab_title">{{ trans('form.deliveryboy_investigation_status.InProgress') }}</span>
                                        <span class="badge badge-light count-assign">{{ $invCounts['In Progress'] ?? '0' }}</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" data-toggle="tab" href="#inv_completed" role="tab" onclick="changeDTTab('datatable-investigation-istarted', 'Done And Delivered')">
                                        <span class="tab_title">{{ trans('form.deliveryboy_investigation_status.Delivered') }}</span>
                                        <span class="badge badge-light count-istarted">{{ $invCounts['Delivered'] ?? '0' }}</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" data-toggle="tab" href="#inv_notCompleted" role="tab" onclick="changeDTTab('datatable-investigation-rwriting', 'Done And Not Delivered')">
                                        <span class="tab_title">{{ trans('form.deliveryboy_investigation_status.NotDelivered') }}</span>
                                        <span class="badge badge-light count-rwriting">{{ $invCounts['Not Delivered'] ?? '0' }}</span>
                                    </a>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div class="tab-content pt-3 pt-md-0 pt-lg-0">
                            <div class="tab-pane active p-3" id="inv_all" role="tabpanel">
                                <table id="datatable-investigation-deliveryboy"
                            ...
                            <div class="tab-pane p-0 p-md-3 p-lg-3" id="inv_inprogress" role="tabpanel">
                                <table id="datatable-investigation-assign"
                                       class="table table-striped table-bordered dt-responsive"
                                       style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    <thead>
                                        <th class="noVis"></th>
                            ...
                            <div class="tab-pane p-3" id="inv_completed" role="tabpanel">
                            ...
                            <div class="tab-pane p-3" id="inv_notCompleted" role="tabpanel">
                                <table id="datatable-investigation-rwriting"
                                       class="table table-striped table-bordered dt-responsive"
                                       style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    <thead>
                                        <th class="noVis"></th>
...
<script>
    var DTable = null;
    var dtSelectedId = "datatable-investigation-deliveryboy";
    var deliveryboyId = null;

    $(document).ready(function () {
        // All Datatable
        var iaDTable = $('#datatable-investigation-deliveryboy').DataTable({
            // dom: 'lBfrtip',
            ...
            ajax: "{{ route('deliveryboy-investigation-list') }}",
            columns: [
        ...
        iaDTable.buttons().container().appendTo('#datatable-investigation-deliveryboy_wrapper .col-md-6:eq(0)');
        ...
        if ($.isArray(params)) {
            ...
            for (var i = 0; i < params.length; i++) {
                ...
                if (nameArr[0] == 'deliveryboy_id') {
                    deliveryboyId = nameArr[1];
                }
    ...
    function changeDTTab(selectedId, status){
        ...
        if ( ! $.fn.DataTable.isDataTable( '#'+selectedId ) ) {
            var url = '{{ route("deliveryboy-investigation-list", ":status") }}';
