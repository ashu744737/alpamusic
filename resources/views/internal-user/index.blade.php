...
@section('title') {{ trans('form.staff') }} @endsection
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-4 col-xs-12">
                            <h4 class="card-title">{{ trans('form.staff') }}</h4>
                        </div>
                        @if(check_perm('staff_create'))
                        <div class="col-12 col-sm-8 col-xs-12 text-left text-sm-right mb-2">
                            <a href="{{ route('staff.create') }}" class="btn btn-primary w-md waves-effect waves-light add-new-btn">{{ trans('form.internal_user.add_staff') }}</a>
                        </div>
                        @endif
                    </div>

                    @include('session-message')
                    @if(check_perm('staff_show'))
                    <div class="row">
                        <div class="col-12">
                            ...
                            <table id="datatable_clients" class="table table-bordered dt-responsive">
                                ...
                                <tr role="row">
                                    ...
                                    <th>{{ trans('form.internal_user.name') }}</th>
                                    <th>{{ trans('form.internal_user.type') }}</th>
                                    <th>{{ trans('form.internal_user.email') }}</th>
                                    <th>{{ trans('form.internal_user.status') }}</th>
            ...
            function ajaxRemoveRecord(record_id) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.status == 'success') {
                            ...
                            Swal.fire(
                                ...
                                (result.message) ? result.message : "{{trans('general.error')}}",
                                "{{ trans('general.error_text') }}"
                            )
                        }
                    ...
                    error: function(response) {
                        ...
                        $('.invalidMessage').find('span.message-content').html("{{trans('general.error')}}");
