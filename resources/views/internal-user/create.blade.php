...
@section('title') {{ trans('form.internal_user.add_user') }} @endsection
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-4 col-xs-12">
                            <h4 class="card-title">{{ trans('form.internal_user.add_user') }}</h4>
                        </div>
                    </div>
                    @include('session-message')
                    <div class="row mt-3">
                        <div class="col-12">
                            <form action="{{ route('staff.store') }}" onSubmit="return dosubmit();" method="POST" id="create_user" class="form needs-validation">
                                @csrf
                                <div class="form-row">
                                    <div class="form-group col-12">
                                        <label for="ContactType" class="ul-form__label">{{ trans('form.internal_user.type') }} <span class="text-danger">*</span></label>
                                        <select onchange="changeusertype(this)" class="form-control" name="user_type_id" id="user_type_id">
                                            <option value="">{{ trans('form.internal_user.field.select_type') }}</option>
                                            @foreach($userTypes as $type)
                                            <option value="{{$type->id}}" {{ old('user_type_id') == $type->id ? 'selected' : '' }}>{{ App::isLocale('hr')?$type->hr_type_name:$type->type_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('user_type_id')
                                ...
                                <div class="form-row  categorydiv {{ old('category') ? 'd-block' : 'd-none' }}">
                                    <div class="form-group col-12">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-6">
                                        <label for="Name" class="ul-form__label">{{ trans('form.internal_user.name') }} <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" id="name" name="name" placeholder="{{ trans('form.internal_user.name') }}" value="{{ old('name') }}"  />
                                        @error('first_name')
                                        <span class="invalid-feedback d-block" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label for="Email" class="ul-form__label">{{trans('form.contact.field.email')}} <span class="text-danger">*</span></label>
                                ...
                                <div class="form-row">
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="password" minlength="6" class="form-control @error('password') is-invalid @enderror" name="password" required autocomplete="new-password" id="userpassword" placeholder="{{ trans('form.enter_password') }}">
                                            @error('password')
                                            <span class="invalid-feedback d-block" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="password" minlength="6" name="password_confirmation" class="form-control" id="userconfirmpassword" required placeholder="{{ trans('form.registration.confirm_password') }}">
                                    </div>
                                </div>
                                <div class="form-row salarydiv {{ old('salary') ? 'd-block' : 'd-none' }}">
                                    <div class="form-group col-12">
                                        ...
                                        <input type="text" class="form-control" id="salary" name="salary" placeholder="{{ trans('form.products_form.salary') }}" value="{{ old('salary') }}"  />
