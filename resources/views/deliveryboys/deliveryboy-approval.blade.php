...
@section('title') {{trans('form.client_approval.title_del')}} @endsection
				...
				<div class="card-body">
					<div class="row align-items-center">
						<div class="col-12 col-sm-12 col-xs-12">
							<h4 class="card-title">{{trans('form.client_approval.client_name_title_del')}} : <span
									class="customer_name"><strong>{{ $user->name ?? '' }}</strong></span></h4>
						</div>
					</div>
					<hr/>
					<form action="{{ route('deliveryboy.approve', $user->id) }}" method="POST" id="client_approve_form"
						  class="form needs-validation">
						@csrf
						<div class="row">
							<div class="col-12">
								<h4 class="section_title mb-3">{{trans('form.client_approval.delivery_price_list')}}</h4>
								<div class="row">
