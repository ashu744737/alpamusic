...
@section('title') {{ trans('form.deliveryboys') }} @endsection
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-4 col-xs-12">
                            <h4 class="card-title">{{ trans('form.deliveryboys') }}</h4>
                        </div>
                        @if(check_perm('deliveryboy_create'))
                        <div class="col-12 col-sm-8 col-xs-12 text-left text-sm-right mb-2">
                            <a href="{{route('deliveryboy.create')}}" class="btn btn-primary w-md waves-effect waves-light add-new-btn">{{ trans('form.registration.deliveryboy.add_deliveryboy') }}</a>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <form id="frm-example" action="#" method="POST">
                                ...
                                <tr role="row">
                                    <th class="noVis"></th>
                                    <th>{{ trans('form.registration.investigator.name') }}</th>
                                    <th>{{ trans('form.registration.investigator.email') }}</th>
                                    <th>{{ trans('form.registration.investigator.family') }}</th>
                                    <th>{{ trans('form.registration.investigator.id_number') }}</th>
                                    <th>{{ trans('form.registration.investigator.website') }}</th>
                                    <th>{{ trans('form.registration.deliveryboy.delivery_areas') }}</th>
                                    <th>{{ trans('form.registration.investigator.phone') }}</th>
                                    <th>{{ trans('form.registration.investigator.address') }}</th>
...
@section('footerScript')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("body").on("click", "#deleteDeliveryboy", function(){
                ...
                Swal.fire({
                    ...
                    text: "{{trans('form.registration.deliveryboy.confirm_delete')}}",
                    type: "warning",
                                ...
                                success: function (response){
                                    if (response.status == 'success') {
                                        Swal.fire({
                                            ...
                                            text: (result.message) ? result.message : "{{trans('form.registration.deliveryboy.deliveryboy_deleted')}}", 
                                            type: "success",
            ...
            if(rows_selected.length > 0){
                Swal.fire({
                    ...
                    if (result.value){
                        ...
                        $.ajax(
                            {
                                url: "{{ route('multidelete-deliveryboy') }}",
                                type: 'POST',
                                dataType: 'json',
                                data: {
                                    "ids": ids,
        ...
        function changeStatus(action, id){
            ...
            var url = "{{ route('deliveryboy.change-status') }}";

            Swal.fire({
                title: "{{trans('general.are_you_sure')}}",
                text: "{{trans('form.registration.deliveryboy.confirm_changestatus')}}",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#34c38f",
                cancelButtonColor: "#f46a6a",
                            ...
                            success: function (response){
                                if (response.status == 'success') {
                                    Swal.fire({
                                        ...
                                        text: (result.message) ? result.message : "{{trans('form.registration.deliveryboy.confirm_statuschanged')}}", 
                                        type: "success",
                                        confirmButtonText: "{{ trans('general.ok') }}",
                                    });
                                    dbDTable.draw();
