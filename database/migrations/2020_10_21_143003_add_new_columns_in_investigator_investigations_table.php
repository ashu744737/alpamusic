<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class AddNewColumnsInInvestigatorInvestigationsTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::table('investigator_investigations', function (Blueprint $table) {
            $table->string('investigator_name')->nullable()->after('investigation_id');
            $table->unsignedInteger('type_of_inquiry')->nullable()->after('investigator_name');
            $table->date('start_date')->nullable()->after('type_of_inquiry');
            $table->time('start_time')->nullable()->after('start_date');
            $table->date('completion_date')->nullable()->after('start_time');
            $table->string('completion_time')->nullable()->after('completion_date');

            $table->index('type_of_inquiry');
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::table('investigator_investigations', function (Blueprint $table) {
            //
        });
    }
}
